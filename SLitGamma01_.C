...
const Int_t type_detector = 4;			 // если =1, то	4 SiPM на торцах, если =2, то 3 SiPM на торцах и другие размеры кристалла, если =3, то модель куба с единичным SiPM на грани, если =4, то две длиные боковые грани

const Int_t type_refrec = 1;			 // если =1, то отражени€ + затухани€, если =2, то только отражени, никаких затуханий нигде нет, если =3, то полное поглощение пленкой
...
if (type_refrec == 1) {
	const Double_t aluminium_diffus = 0.01;		   // дол€ которые рассеивютс€, а не отражаютс€	   0.01
	...
	const Double_t aluminium_diffus = 0.0;		   // дол€ которые рассеивютс€, а не отражаютс€
	const Double_t AluminiumSupplAbs = 0.0;		   // дополнительна€ абсорбци€	   0.1

}
else if (type_refrec == 3) {
	const Double_t aluminium_diffus = 0.0;		   // дол€ которые рассеивютс€, а не отражаютс€
	const Double_t AluminiumSupplAbs = 1.0;		   // дополнительна€ абсорбци€	   0.1
...
if (type_detector == 4) {
	...
	const Double_t crys_dx = 18.0 / 2.0;		   //in cm
	const Double_t crys_dy = 0.65 / 2.0;
	const Double_t crys_dz = 4.0 / 2.0;
...
if (type_detector == 1 || type_detector == 2) {
	...
	const Double_t apd_dx = 18.0 / 2.0;
	const Double_t apd_dy = 0.65 / 2.0;
	const Double_t apd_dz = 0.145 / 2.0;		// всего SiPM
	//const Double_t apd_dz = 0.03 / 2.0;			// фоточувствительной области
}
...
if (type_refrec == 1) {
	const Double_t wid = 0.005;	 //0,05 мм=0,005 у нас, было по умолчани€ 0,1
	Double_t ws2 = 0.1 * wid;

}
else if (type_refrec == 2) {

	const Double_t wid = 0.000001;	 //0,03 мм=0,003 у нас, было по умолчани€ 0,1
	Double_t ws2 = 0.1 * wid;
...
// Dimensions of TOP
//
const Double_t top_dx = crys_dx + 6.0;
const Double_t top_dy = crys_dy + 6.0;
...
if (type_detector == 1) {
	...
	TGeoCompositeShape* rev_shape = new TGeoCompositeShape("REV", "CRYSTALWS+APDWS_1:t1_1+APDWS_2:t1_2");
...
if (FromFace) {
	...
	if (k == 1) {
		...
		if (type_detector == 4) {
			TVector3 emisdir(1.0, 0.0, 0.0); // axis around which photons are emitted
			TVector3 facedir(1.0, 0.0, 0.0); // axis to follow to reach face of emission
			TVector3 source(0.0, 0.0, 0.0);  // irrelevant
